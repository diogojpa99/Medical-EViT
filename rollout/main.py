...
def generate_attention_map(model,tensor_image,head_fusion="mean",discard_ratio=0.9,method="rollout"):
    ...
    if method == "rollout":
        ...
        grad = model.transformer.layers[-1][0].fn.fn.get_attn_gradients()
        cam = model.transformer.layers[-1][0].fn.fn.get_attn()
        cam = cam[0].reshape(-1, cam.shape[-1], cam.shape[-1])
        grad = grad[0].reshape(-1, grad.shape[-1], grad.shape[-1])
        cam = grad * cam
        cam = cam.clamp(min=0).mean(dim=0)
        ...
        cam = model.transformer.layers[3][0].fn.fn.get_attn()
        cam = cam[0].reshape(-1, cam.shape[-1], cam.shape[-1])
        cam = cam.clamp(min=0).mean(dim=0)
        cam = cam[0, 1:]
        return cam
